...
import {writeExcelFile} from '../JS/writeExcelFile'
...
async function initInputData(file) {
  ...
  if (!inputData.error) {
    ...
    listCompany = listCompany.filter(el => el.id.length > 0 && el.query)
...
const onClickAction = async () => {
  state.countRecords > 0 ?  writeExcelFile(await getSuggestions()): console.log("Файл не выбран");
 // startProgress()
...
const getSuggestions = async (request) => {
  ...
  i+=1;
    const sug = await postRequest(request.query.trim().replace('"', "").replace('\t',' ') );
    startProgress(i)
  ...
  for (const i of sug.suggestions) {
      listCompanyInfo.push({
          ...i,
          request,
          countResult: sug.suggestions.length //количество ответов на запрос
      });
  }
} else {
  listCompanyInfo.push( {
